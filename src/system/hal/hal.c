...
static void vesa_put_pixel(int x, int y, uint32_t color) {
    if (x < 0 || y < 0 || (uint32_t)x >= screen_width || (uint32_t)y >= screen_height) return;
...
static void mouse_state(int *x, int *y, uint8_t *buttons) {
    ...
    if (y) *y = mouse_y;
    if (buttons)
        *buttons = (mouse_left_button  ? 1 : 0) | (mouse_right_button ? 2 : 0);
}
...
static int ata_block_read(hal_block_ops_t *self, uint64_t lba, uint32_t count, void *buf) {
    read_sectors_ata_pio((uintptr_t)buf, lba + self->lba_offset, count);
    return (int)count;
}

static int ata_block_write(hal_block_ops_t *self, uint64_t lba, uint32_t count, const void *buf) {
