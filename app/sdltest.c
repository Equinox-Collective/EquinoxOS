...
int main(int argc, char *argv[]) {
    ...
    /* На EquinoxOS нет OpenGL — форсируем software-путь, чтобы SDL не пытался
     * создать GPU/texture framebuffer. */
    ...
    while (running) {
        ...
        /* 1. Анимированный градиент на фоне */
        ...
        /* 2. Интерактивный квадрат в позиции курсора */
        SDL_Rect rect;
        rect.x = mouse_x - 20;
        rect.y = mouse_y - 20;
        rect.w = 40;
        rect.h = 40;
        if (mouse_pressed) {
            SDL_SetRenderDrawColor(renderer, 0, 255, 100, 255); /* зелёный при клике */
        } else {
